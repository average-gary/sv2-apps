...
pub struct ChannelManager {
    ...
    // Coinbase outputs. Wrapped in `SharedLock` because the xpub derivator
    // may rotate the active script at runtime when a block is found.
    pub(crate) coinbase_outputs: SharedLock<Vec<u8>>,
    // Last new prevhash.
...
impl ChannelManager {
    ...
    pub async fn new(
        ...
        // We use its current_script_pubkey() for the initial coinbase_outputs so
        // we use the correct address from the persisted index (or start_index)
    ...
    ///
    /// This should be called after a block is found. It derives the next
    /// sequential address, persists the index to disk, and updates the
    /// internal coinbase_outputs for future templates.
    ///
    ...
    pub fn rotate_coinbase_address(&self) {
        ...
        match derivator.next_script_pubkey() {
            Ok(new_script) => {
                ...
                if let Err(e) = self.coinbase_outputs.set(new_outputs) {
                    error!("Failed to update coinbase outputs: {:?}", e);
                }
            }
            Err(e) => {
