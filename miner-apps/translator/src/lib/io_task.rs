...
//! the [`Sv2Frame`] channels consumed by the rest of the translator's upstream
//! code.
//!
//! Like JDC, the translator participates in **fallback coordination**: when an
//! upstream fault triggers a fallback, the bridge tasks must register with the
//! [`FallbackCoordinator`] so the fallback driver can wait for them to drain
//! before dialing the next upstream candidate. This mirrors the registration
//! that the legacy `spawn_io_tasks` performed against `NoiseTcpReadHalf` /
//! `NoiseTcpWriteHalf` before this module switched to the transport-agnostic
//! [`ConnPair`].
...
/// existing translator code without changing the inner [`Sv2Frame`] channel
/// shape consumed by the message handlers, we spawn two thin bridge tasks
/// that translate between [`StandardEitherFrame<Message>`] (what the transport
/// speaks) and [`Sv2Frame`] (what the rest of the translator code speaks).
...
/// semantics that the legacy `spawn_io_tasks` had on the translator's
